...
void ppHinhHoc(RangBuoc Rang_buoc[], int n, float c1, float c2) {
	...
	// In ket qua
	printf("\nPhuong toi an uu la: (%.3f; %.3f)", best_point.x, best_point.y);
	printf("\nGia tri toi uu z = %.3f", max);
}

void docDuLieuHinhHoc(RangBuoc Rang_buoc[], int *n_ptr, float *c1_ptr, float *c2_ptr) {
	fprintf(stderr, "Nhap so rang buoc dang ax + by <= c: "); scanf("%d", n_ptr);
	
	int i;
	for (i = 0; i < *n_ptr; i++) {
		fprintf(stderr, "Nhap a, b, c cho rang buoc %d: ", i + 1);
		scanf("%f%f%f", &Rang_buoc[i].a, &Rang_buoc[i].b, &Rang_buoc[i].c);
	}
	
	fprintf(stderr, "Nhap c1, c2 cho ham muc tieu z = c1*x + c2*y: "); scanf("%f%f", c1_ptr, c2_ptr);
}

void docFile_HinhHoc(RangBuoc Rang_buoc[], int *n_ptr, float *c1_ptr, float *c2_ptr) {
	char s[100];
	fprintf(stderr, "Nhap ten file: "); scanf("%s", s);
	fprintf(stderr, "\n--------------------");
	FILE *fptr;
	fptr = fopen(s, "r");
	fscanf(fptr, "%d", n_ptr);
...
void xuatHamMucTieu(float c[], int n) {
	...
	for (int j = 2; j <= n - 1; j++) {
		if (c[j] != 0) {
			if (check == 1) {
				if (c[j] > 0) printf("+ ");
				else if (c[j] < 0) printf("- ");
			}
			else if (check == 0) {
				if (c[j] < 0) printf("-");
				check = 1;
			}
			if (c[j] != 1) printf("%.3f*", fabs(c[j]));
			printf("x%d ", j);
		}
	}
}
...
void xuatRangBuoc(float A[100][100], int m, int n) {
	...
	for (i = 1; i <= m - 1; i++) {
		...
		if (A[i][1] != 0) {
			if (A[i][1] < 0) printf("-");
			if (A[i][1] != 1) printf("%.3f*", fabs(A[i][1]));
			printf("x1 ");
			check = 1;
		}
		for (int j = 2; j <= n - 1; j++) {
...
void xuatBang(float A[100][100], int m, int n) {
	...
	for (i = 1; i <= m; i++) {
		...
		printf("\n");
		for (j = 1; j <= n; j++) printf("|%-16.3f", A[i][j]);
		printf("|");
	}
	printf("\tdelta[j]\n");
	for (j = 1; j <= n; j++) printf("-----------------");
...
void nhapDuLieu(float A[100][100], int *m_ptr, int *n_ptr,int acb[], int acb_vi_tri[], float c[], float cB[]) {
	fprintf(stderr, "Nhap so an: "); scanf("%d", n_ptr);
	fprintf(stderr, "Nhap so phuong trinh: "); scanf("%d", m_ptr);
	(*m_ptr)++; // tạo thêm cột cho các hệ số tự do 
	(*n_ptr)++; // tạo thêm hàng cho các số kiểm tra
	
	int i;
	fprintf(stderr, "Nhap ma tran A bieu dien he phuong trinh tuyen tinh:\n");
	nhapMaTran(A, *m_ptr, *n_ptr); 
	for (i = 1; i <= (*n_ptr) - 1; i++) {
		fprintf(stderr, "Nhap he so cua x%d trong ham muc tieu: ", i);
		scanf("%f", &c[i]);
	}
}
...
int main() {
	int pp;
	fprintf(stderr, "PBL1: BAI TOAN QUY HOACH TUYEN TINH (LINEAR PROGRAMMING)\n\n====================\n\nCac thanh vien (Nhom 22):\n\n1. Phan Phu Thinh\n\n2. Tran Ngoc Duy\n\n3. Nguyen Danh Ngon\n\n");
	fprintf(stderr, "====================\n");
	fprintf(stderr, "\nCac phuong phap:\n");
	fprintf(stderr, "1. Hinh hoc.\n");
	fprintf(stderr, "2. Don hinh.\n\n");
	fprintf(stderr, "====================\n");
	fprintf(stderr, "Chon phuong phap: "); scanf("%d", &pp);
	switch (pp) {
		case 1: {
			int n;
			float c1, c2;
			...
			fprintf(stderr, "\nNhap du lieu:\n");
			fprintf(stderr, "1. Qua ban phim.\n");
			fprintf(stderr, "2. Qua file.\n");
			fprintf(stderr, "--------------------\n");
